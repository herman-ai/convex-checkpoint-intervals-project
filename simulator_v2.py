...
def simulate_one_interval_useful_work_hazard(
    ...
    Assumptions:
    - 1 unit of useful work takes 1 unit of wall-clock time when running.
    - Failure hazard depends on useful work position, not wall-clock time.
    - On failure, progress within the interval is lost and the same useful-work segment is retried.
    """
    a = start_work
    ...
    while True:
        ...
        # failure occurs inside the useful-work segment
...
def monte_carlo_schedule_useful_work_hazard(
    ...
    stderr_runtime = float(np.std(runtimes, ddof=1) / np.sqrt(num_trials)) if num_trials > 1 else 0.0
